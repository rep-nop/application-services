...
async function oauthCommand(emailAddr, password, fxaAuthUrl, oauthFlowUrl) {
    ...
    const nightmare = Nightmare({
        // False if we should run headless, set to true to help debug if this script is
        // failing on command == "oauth". (Also, `DEBUG=nightmare` in the environment
        // adds logging and may help debugging as well).
        show: false,
    ...
    try {
        let needConfirmation = await nightmare.goto(oauthFlowUrl)
...
async function clearRestmail(emailAddr) {
    ...
    await request(mailboxUrl, { method: "DELETE" });
}
...
async function createCommand(emailAddr, password, authUrl) {
    ...
    let { uid } = await postJSON(`${authUrl}/v1/account/create`, {
        email: emailAddr,
        authPW: genAuthPW(emailAddr, password),
    });
