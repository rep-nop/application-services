...
internal interface LibPlacesFFI : Library {
    ...
    fun places_connection_new(db_path: String, encryption_key: String): RawPlacesConnection?

    /** Returns JSON string, which you need to free with places_destroy_string */
    fun places_note_observation(conn: RawPlacesConnection, json_observation_data: String)

    /** Returns JSON string, which you need to free with places_destroy_string */
    fun places_query_autocomplete(conn: RawPlacesConnection, search: String, limit: Int): Pointer?
