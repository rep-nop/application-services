...
/// [`ffi_support::destroy_c_string`] method in this module, or by dropping the underlying
/// [`std::ffi::CString`] after recovering it via `std::ffi::CString::from_raw()`.
///
...
/// Variant of [`ffi_support::rust_string_to_c`] which takes an Option, and returns null for None.
#[inline]
...
/// Free the memory of a string created by [`rust_string_to_c`](ffi_support::rust_string_to_c) on
/// the rust heap. If `c_string` is null, this is a no-op.
///
...
/// use [`rust_string_from_c`](ffi_support::rust_string_from_c) instead, which will do a lossy conversion).
///
/// If you actually need an owned rust `String`, you're encouraged to use
/// [`rust_string_from_c`](ffi_support::rust_string_from_c), which, as mentioned, also behaves
/// better in the face of invalid UTF-8.
///
/// ## Safety
///
...
/// ## Panics
///
/// Panics if it's argument is null, see [`ffi_support::opt_rust_str_from_c`] for a variant
/// that returns None in this case instead.
///
/// Note: This means it's forbidden to call this outside of a `call_with_result` (or something else
/// that uses `std::panic::catch_panic`), as it is UB to panic across the FFI boundary.
#[inline]
    ...
    if !c_string.is_null() {
        match CStr::from_ptr(c_string).to_str() {
            ...
            Err(e) => {
                ...
                error!("[Bug] Invalid UTF-8 was passed to rust! Report this! {:?}", e);
                Some("")
            }
        }
    } else {
...
/// ## Panics
///
/// Panics if it's argument is null. See also [`ffi_support::opt_rust_string_from_c`], which
