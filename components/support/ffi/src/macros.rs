...
/// Implements IntoFfi for the provided types (more than one may be passed in) by allocating `$T` on
/// the heap as an opaque pointer. See the `IntoFfi` documentation for information.
///
/// You may instead want to use `expose_opaque!` which is a simple helper that calls both this and
/// `define_destructor!`.
#[macro_export]
...
/// JSON using `serde_json`. See the `IntoFfi` documentation for more information.
///
/// They must implement `serde::Serialize`.
///
/// ## Panics
///
...
///
/// It simply expands to a `#[no_mangle] pub unsafe extern "C" fn` which wraps this modules
/// `destroy_c_string` function.
///
/// ## Caveats
///
/// If you're using the multiple separately compiled rust libraries in your application,
/// it's critical that you are careful to only ever free strings allocated by a Rust library using
...
/// Helper for the common case where you want to both `define_box_destructor!($T, $destructor_name)`
/// and `implement_into_ffi_by_pointer!($T)`
///
/// ## Example
///
/// ```rust
...
/// struct CoolType(Vec<i32>);
///
/// expose_opaque!(CoolType, mylib_destroy_cooltype);
/// ```
#[macro_export]
macro_rules! expose_opaque {
    ($T:ty, $destructor_name:ident) => {
        define_box_destructor!($T, $destructor_name);
        implement_into_ffi_by_pointer!($T);
    };
}
