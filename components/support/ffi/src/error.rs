...
impl ErrorCode {
    ...
    pub fn new(code: i32) -> Self {
        assert!(code != ErrorCode::PANIC.0 && code != ErrorCode::SUCCESS.0,
            "Error: The ErrorCodes `{:?}` (got {}) are all reserved. You may use the associated \
            constants on this type (`ErrorCode::PANIC`, etc) if you'd like instances of those error \
            codes.", [ErrorCode::PANIC.0, ErrorCode::SUCCESS.0], code);
...
///
/// While this pattern is not ergonomic in Rust, it offers two main benefits over something more
...
/// of the FFI will need to know it's layout.
///
...
/// #### The `message` field.
///
/// This is a string containing some amount of additional information about the error. If the `code`
/// property is nonzero, there should always be an error message. Otherwise, this should will be
/// null.
///
...
///
/// Typically, this object is managed on the other side of the FFI, which means you must expose a
/// function to release the resources of `message`which can be done easily using the
